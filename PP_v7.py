...
rpt_part_payment_02 = cbs_ar_invoice.select(col("INVOICE_ID"),
                              col("ACCT_ID"),
                              col("BILL_CYCLE_ID").alias("BILL_CYCLE_ID2"),
                              col("PRI_IDENTITY"),
                              col("INVOICE_AMT"),
                              col("OPEN_AMT"),
                              (coalesce(col("INVOICE_AMT"),lit(0))/lit(1000)).alias("INVOICE_FEE2"),
                              (coalesce(col("OPEN_AMT"),lit(0))/lit(1000)).alias("OPEN_FEE2")
                     ).where(
                              (col("INVOICE_TYPE")==lit("5")) \
                             & ((col("AOD_STATUS") != lit("Y")) | (col("AOD_STATUS").isNull()))\
                             & (col("TRANS_TYPE")==lit("BLL"))\
                             & (col("OPEN_AMT") >= lit(1000000))\
                             & (col("INVOICE_AMT") != col("OPEN_AMT"))
                            ).withColumn("TRANS_AMT",((coalesce(col("INVOICE_AMT"),lit(0))) -\
                                                (coalesce(col("OPEN_AMT"),lit(0))))/lit(1000))

rpt_part_payment_03 = rpt_part_payment_02.join(rpt_part_payment_01, \
                                 rpt_part_payment_02.ACCT_ID == rpt_part_payment_01.CBS_ACCT_ID, \
                                 'left_outer').select(col("INVOICE_ID").alias("INVOICE_ID2"),col("INVOICE_FEE2")\
                                                      ,col("OPEN_FEE2"),col("BILL_CYCLE_ID2"),\
                                                      col("TRANS_AMT"),col("Province_id"))

rpt_part_payment_03_1_A = rpt_part_payment_03.join(cbs_ar_invoice, rpt_part_payment_03.INVOICE_ID2 == cbs_ar_invoice.INVOICE_ID , 'inner')\
    .where(
            (col("INVOICE_AMT") == col("OPEN_AMT")) &\
            (col("INVOICE_TYPE") == lit("5")) &\
            (col("trans_type") == lit("BLL")) &\
            ((col("AOD_STATUS") != lit("Y")) | (col("AOD_STATUS").isNull())) &\
            (col("BILL_CYCLE_ID")[0:6] < PARAM_DATA_MONTH)
          ).select(col("BILL_CYCLE_ID"), col("Province_id"),col("INVOICE_ID2")\
                   ,col("TRANS_AMT"),col("INVOICE_FEE2")
          ).groupBy(
                col("BILL_CYCLE_ID"),\
                col("Province_id")\
        ).agg(F.count(col("INVOICE_ID2")).alias("CHANGE_CNT"), F.sum(col("TRANS_AMT"))\
              .alias("CHANGE_PAY_FEE"),F.sum(col("INVOICE_FEE2")).alias("CHANGE_INV_FEE"))

rpt_part_payment_03_1 = rpt_part_payment_03_1_A.select(col("BILL_CYCLE_ID"),col("Province_id")\
                        ,col("CHANGE_CNT"),col("CHANGE_PAY_FEE"),col("CHANGE_INV_FEE"),lit(0).alias("PART_CNT")\
                        ,lit(0).alias("PART_PAY_FEE"),lit(0).alias("PART_INV_FEE"),lit("1").alias("PAID_FLAG"))

rpt_part_payment_03_2_A = rpt_part_payment_03.select(col("BILL_CYCLE_ID2"),col("Province_id").alias("Province_id2"),\
    col("TRANS_AMT").alias("TRANS_AMT2"),col("INVOICE_FEE2"))\
    .groupBy(col("BILL_CYCLE_ID2"), col("Province_id2")\
    ).agg(F.count(col("BILL_CYCLE_ID2")).alias("PART_CNT2"), F.sum(col("TRANS_AMT2"))\
              .alias("PART_PAY_FEE2"),F.sum(col("INVOICE_FEE2")).alias("PART_INV_FEE2"))
...
'''
'''
rpt_part_payment_03_2_all = rpt_part_payment_03_2_A.join(t_m_rpt_part_payment_d, \
         (t_m_rpt_part_payment_d.BILL_CYCLE_ID == rpt_part_payment_03_2_A.BILL_CYCLE_ID2)\
            & (t_m_rpt_part_payment_d.Province_id == rpt_part_payment_03_2_A.Province_id2),\
                             'full_outer')\
    .drop("CHANGE_CNT","CHANGE_PAY_FEE","CHANGE_INV_FEE","PAID_FLAG")\
    .withColumn("BILL_CYCLE_ID",coalesce(col("BILL_CYCLE_ID"),col("bill_cycle_id2")))\
    .withColumn("PROVINCE_ID",coalesce(col("PROVINCE_ID"),col("province_id2")))\
    .withColumn("PART_CNT",coalesce(col("PART_CNT2"),lit("0")))\
    .withColumn("PART_PAY_FEE",coalesce(col("PART_PAY_FEE2"),lit("0")))\
    .withColumn("PART_INV_FEE",coalesce(col("PART_INV_FEE2"),lit("0")))\
    .withColumn("CHANGE_CNT",lit(0))\
    .withColumn("CHANGE_PAY_FEE",lit(0))\
    .withColumn("CHANGE_INV_FEE",lit(0))\
    .withColumn("PAID_FLAG",lit(1))

rpt_part_payment_03_2 = rpt_part_payment_03_2_all.select(
    col("bill_cycle_id"),\
    col("province_id"),\
    col("CHANGE_CNT"),\
    col("CHANGE_PAY_FEE"),\
...
u1 = rpt_part_payment_03_1.unionAll(rpt_part_payment_03_2)

t_m_rpt_part_payment_d = u1.select(col("bill_cycle_id"),col("province_id"),col("PAID_FLAG"),\
                                   col("PART_CNT"),col("PART_PAY_FEE"),col("PART_INV_FEE"),\
                                   col("CHANGE_CNT"),col("CHANGE_PAY_FEE"),col("CHANGE_INV_FEE")
                                  )\
    .groupBy(
