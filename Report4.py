...
class RemainedDebt:
    ...
    def run_spark(self):
        ...
        #===========================================================#
        # step_1_12 = t_l_chr_m.alias('a') \
        #     .join(t_m_rpt_charity_sum_5.alias('b').where(col('b.invoice_status') != lit('C')),
        #           (
        #                   (col('a.bill_cycle_id') == col('b.bill_cycle_id'))
        #                   & (col('a.org_name') == col('b.org_name'))
        #                   & (col('a.acct_id') == col('b.acct_id'))
        ...
        #           ), 'left_outer') \
        #     .select(
        #     coalesce(col('b.bill_cycle_id'), col('a.bill_cycle_id')).alias('bill_cycle_id'),
        #     coalesce(col('b.acct_id'), col('a.acct_id')).alias('acct_id'),
        #     coalesce(col('b.org_name'), col('a.org_name')).alias('org_name'),
        #     coalesce(col('b.org_code'), col('a.org_code')).alias('org_code'),
        #     coalesce(col('b.province_id'), col('a.province_id')).alias('province_id'),
        #     coalesce(col('b.debit_fee'), col('a.debit_fee')).alias('debit_fee'),
        #     coalesce(col('b.invoice_status'), col('a.invoice_status')).alias('invoice_status'),
        # )
        #
        # step_3_12 = t_m_rpt_charity_sum_5.alias('a').join(t_l_chr_m.alias('b'),
        #                                                   (
        #                                                           (col('a.bill_cycle_id') == col('b.bill_cycle_id'))
        #                                                           & (col('a.org_name') == col('b.org_name'))
        #                                                           & (col('a.acct_id').cast('string') == col(
        #                                                       'b.acct_id').cast('string'))
        #                                                           & (col('a.org_code') == col('b.org_code'))
        #                                                   ), 'left_anti').select(
        #     col('a.bill_cycle_id'), col('a.acct_id'),
        #     col('a.org_name'), col('a.org_code'),
        #     col('a.province_id'), col('a.debit_fee'),
        #     col('a.invoice_status')
        # ).where(col('a.invoice_status') == lit('C'))
        #
        ...
        # #     '''
        # #     select
        # #     bill_cycle_id, acct_id,
        # #     org_name, org_code,
        # #     province_id, debit_fee,
        # #     invoice_status
        # #     from t_m_rpt_charity_sum_5
        # #     where (bill_cycle_id, acct_id, org_name, org_code) not in (select bill_cycle_id, acct_id, org_name, org_code from t_l_charity_m)
        # #     and invoice_status = 'C'
        # #     '''
        # #     )
        #
        # t_m_rpt_charity_sum_12 = step_1_12.union(step_3_12)
        # inter_output = t_m_rpt_charity_sum_12.select(
        #     lit(b).alias('DATA_MONTH'),
        #     col('bill_cycle_id').alias('BILL_CYCLE_ID'),
        #     col('acct_id').alias('ACCT_ID'),
        #     col('province_id').alias('PROVINCE_ID'),
        ...
        #     date_format(current_timestamp(), 'yyyy/MM/dd HH:mm:ss').alias('PROCS_TIME')
        # )
        # # ba tavajoh be sohbat haye ba mehrad be in natije reside shod ke insert avalie dar dakhele sand na dorost ast va
        # # edemaye script darvaghe join 12 ba 7 mibashad
        # t_m_rpt_charity_sum_8_1 = t_m_rpt_charity_sum_12.alias('a') \
        #     .join(t_m_rpt_charity_sum_7.alias('b'),
        #           (
        #                   (col('a.bill_cycle_id') == col('b.bill_cycle_id'))
        #                   & (col('a.acct_id') == col('b.acct_id'))
